...
function buildUrl() {
  if (!SHEET_ID || !API_KEY) {
    throw new Error(
      "Faltan GOOGLE_SHEET_ID o GOOGLE_SHEETS_API_KEY en las variables de entorno."
    );
  }
...
export function buildDashboardData(values) {
  ...
  return {
    ...
    filters: {
      ...
      currencies: [...new Set(records.map((r) => r.currency))].sort(),
      months: monthlyWithAccumulated.map((m) => ({
        value: m.monthKey,
        label: m.label,
      })),
    },
